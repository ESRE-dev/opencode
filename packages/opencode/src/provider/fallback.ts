...
export namespace ProviderFallback {
  // Details handed to the onFallback hook when a transient error triggers a
  // cross-provider retry. The caller owns logging and toast publishing so this
  // module stays free of the Effect event/runtime machinery.
  ...
  export function middleware(
    ...
    onFallback?: (info: Activation) => void,
  ): LanguageModelMiddleware {
    const activate = (err: unknown) =>
      onFallback?.({
