...
test("reply - can answer child session question from parent context", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(pending[0].sessionID).toBe("ses_child")
...
test("rejectSession - only rejects questions for the specified session", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      await Question.rejectSession("ses_child")
      ...
      const pending = await Question.list()
      expect(pending.length).toBe(1)
      expect(pending[0].sessionID).toBe("ses_other")
    },
  })
})
...
test("reply - resolving parent question does not affect child questions", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const pending = await Question.list()
      const req = pending.find((p) => p.sessionID === "ses_parent")!
      await Question.reply({
        requestID: req.id,
        answers: [["A"]],
      })
      ...
      expect(remaining[0].sessionID).toBe("ses_child")
    },
  })
})
