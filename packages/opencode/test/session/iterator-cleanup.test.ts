...
/**
 * Unit tests for the async iterator cleanup pattern.
...
describe("iterator cleanup", () => {
  test("return() called on abort", async () => {
  ...
  test("return() called on normal completion", async () => {
  ...
  test("return() error suppressed", async () => {
    const iter: AsyncIterator<number> = {
      async next() {
        return { done: true as const, value: undefined }
      },
  ...
  test("return() tolerates exhausted iterator", async () => {
    const { iter, spy } = mock([])
  ...
  test("return() called on thrown error", async () => {
    const { iter, spy } = mock([1, 2, 3])
    const aborted = new Promise<never>(() => {})
  ...
  test("pre-aborted signal triggers immediate rejection and return()", async () => {
    const { iter, spy } = mock([1, 2, 3])
    const controller = new AbortController()
    controller.abort()
    const aborted = new Promise<never>((_, reject) => {
    ...
    try {
      while (true) {
        const { done } = await Promise.race([iter.next(), aborted])
        if (done) break
        controller.signal.throwIfAborted()
      }
    } catch {
